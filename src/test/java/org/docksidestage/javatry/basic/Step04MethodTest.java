...
public class Step04MethodTest extends PlainTestCase {
    ...
    // #1on1: いいね、メソッドの定義順が呼び出し側の呼び出し順序と一緒で直感的でわかりやすい (2026/09/08)
    // なんだかんだこういった感覚的なところに人間影響を受けるので、jfluteはコードを理解しやすいと感じた。
    // まず、呼び出し側の近くに集めよう。
    // そして、呼び出し順序を意識した配置。
    // もちろん、これは方法論の一つなので、他の考え方で統一されていればそれでいいけど...
    // 多少なり、その辺意識してメソッドの配置を考えていって欲しい。
    //
    // 単純に一番下に追加される問題話。
    // おじゃまします感で既存クラスにメソッドを追加してしまう。
    // 元々整ってたクラスが、五月雨追加/修正でごちゃごちゃになってしまうケース。
    // 既存クラスの作りのコンセプトを把握して、メソッド追加/修正、をして欲しい。
    // そして、おじゃまします感ではなく、堂々と踏み込んで、そのクラスのコードデザインに責任を持って欲しい。
    //
    // 既存コードで微妙だと思ったら？
    // $修正規模による、ちょこっとだったらやれるかも
    // リファクタリングチケット置き去りジレンマ話。
